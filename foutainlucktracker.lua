...
local EMBED_COLOR = 0x7A00FF -- 💜 Main embed color
local EXTEND_COLOR = 0x00FFCC -- 💚 Extend alert embed
local POLL_RETRY = 1
...
local function sendOrEditMain(embed)
	...
	-- fallback: gửi mới nếu chưa có hoặc edit fail
...
local function sendExtendAlert(playerName, newInfo, delta)
	local embed = {
		...
		color = EXTEND_COLOR,
		fields = {
			{name=string.format("🍀 %s",playerName),value=string.format("🕒 +%d phút (%s)",math.floor(delta/60),newInfo.discordCountdown),inline=false}
		},
		timestamp=os.date("!%Y-%m-%dT%H:%M:%S.000Z"),
		thumbnail={url=EXTEND_THUMBNAIL},
		footer={text="Fountain Extended — GCC Tracker",icon_url=BOT_AVATAR}
	}
	local req = (http_request or request or syn.request)
...
task.spawn(function()
	...
	for _, entity in ipairs(entitiesParent:GetChildren()) do
		...
		if expireLabel and expireLabel:IsA("TextLabel") then
			...
			if info then
				...
				table.insert(playerFields,{name=" 🎮 Player:  "..name,value="⏳ "..info.discordCountdown,inline=false})

				-- Theo dõi tăng thời gian
	...
	-- 🎮 Build main embed
	local embed = {
		title = "⚔️ **GCC Arena — Luck Fountain Status**",
	...
	-- Gửi hoặc edit main embed
	sendOrEditMain(embed)
	print("[LuckWatcher] ✅ Sent/Edited main embed to Discord.")
end)
