if false then Nexus,syn,NoShutdown,Krnl,WebSocket,messagebox,identifyexecutor,Nexus_Version,_TARGETFPS,_PERF,newcclosure,Command,identifyexecutor,getgenv = nil end
...
if not game:IsLoaded() then
	task.delay(40, function()
		...
		if not game:IsLoaded() then
			local errorChannel = getgenv().ErrorChannel
			if errorChannel then
				errorChannel:Fire("Not loaded")
			end
			
			task.wait(10)

			return game:GetService("TeleportService"):Teleport(1730877806, game:GetService("Players").LocalPlayer)
		end
		...
		if Code >= Enum.ConnectionError.DisconnectErrors.Value then
			local errorChannel = getgenv().ErrorChannel
			if errorChannel then
				errorChannel:Fire("Disconnected.")
			end
...
local GuiService = nil or game:GetService'GuiService'
...
do -- Connections
	GuiService.ErrorMessageChanged:Connect(function(errorMessage)
		...
		if Code >= Enum.ConnectionError.DisconnectErrors.Value then
			...
			if errorMessage == "DisconnectClientInitiated" then
				return
			end
			
			local errorChannel = getgenv().ErrorChannel
			if errorChannel then
				errorChannel:Fire(errorMessage)
			end
